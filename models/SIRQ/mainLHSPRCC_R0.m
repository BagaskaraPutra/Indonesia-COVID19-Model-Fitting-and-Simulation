...
% Variable Labels
y_var_label = 'R0'; %model.allStateName;
...
%     x
    LHSmatrix(x,:);
    [t,y]=ode15s(@(t,y)f(model,t,LHSmatrix,x),tspan,y0,[]); 
    A=[t y]; % [time y]
    
    %% Save only the outputs at the time points of interest [time_points]:
    %% MORE EFFICIENT
    model.R0.lhs(:,x) = A(time_points+1,2);
end
%% Save the workspace
% CALCULATE PRCC
[prcc sign sign_label hFig]=PRCC_R0(LHSmatrix,model,time_points,PRCC_var,alpha,'horizontal');
...
if(savePrompt == 'Y' || savePrompt == 'y')
    ...
    saveDir = ['results/' 'R0/' timeString]; mkdir(saveDir);
    save([saveDir '/PRCCof_R0.mat'],'-regexp','^(?!(hFig)$).');
    fprintf(['MAT file saved in ' saveDir '\n']);
    saveDir = ['results/' 'R0/' timeString '/tables']; mkdir(saveDir);
    ...
    saveDir = ['results/' 'R0/' timeString '/figures'];
    mkdir(saveDir);
