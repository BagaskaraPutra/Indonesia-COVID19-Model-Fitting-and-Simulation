%% Model SIR(modified) Surabaya per Kebijakan:
...
for i=1:numel(k)
    for j=1:numel(model.allStateName)
        lama.(model.allStateName{j}) = zeros(1,1);
...
if(softwareName == 'matlab')
    gabung = getDataModelSIR('COVID19_Surabaya.xls'); %[EDITABLE Matlab]
else
    gabung = getDataModelSIR('COVID19_Surabaya.csv'); %[EDITABLE Octave]
end
...
keteranganSimulasi = ['MulaiFitting' datestr(k{rfi}.timeFit{1},'yyyy-mm-dd') ...
