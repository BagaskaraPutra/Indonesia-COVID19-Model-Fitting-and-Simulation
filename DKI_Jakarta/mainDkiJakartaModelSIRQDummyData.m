...
%By: Bagaskara P.P., Last Modified: 2020-10-17
...
%% Simulate manually set parameters [EDITABLE]

for i=1:numel(k) 
    k{i}.paramDummy = [0.0498476834651542,0.0261007589456534,0.0695170914082141,0.00183045141846911];
%     k{i}.paramDummy(find(strcmp(model.paramName, 'beta'))) = 0.0498476834651542;
%     k{i}.paramDummy(find(strcmp(model.paramName, 'r_I_Q'))) = 0.0261007589456534;
%     k{i}.paramDummy(find(strcmp(model.paramName, 'gamma'))) = 0.0695170914082141;
%     k{i}.paramDummy(find(strcmp(model.paramName, 'muI'))) = 0.0018304514184691;
end
...
%% Make simulated dummy data into fitting data
...
%     k{i}.lbParam(find(strcmp(model.paramName, 'r_I_Q'))) = 0;
...
%     k{i}.ubParam(find(strcmp(model.paramName, 'beta'))) = 2.5*2e-1;
%     k{i}.paramEst(find(strcmp(model.paramName, 'r_I_Q'))) = 2.5*2e-1;
%     k{i}.ubParam(find(strcmp(model.paramName, 'gamma'))) = 2.5*1/15;
%     k{i}.ubParam(find(strcmp(model.paramName, 'muI'))) = 2.5*1/20;
end
...
%     k{i} = rmfield(k{i},'paramEst');
    k{i} = rmfield(k{i},'timeSim');
    k{i} = rmfield(k{i},'Test');
    k{i} = rmfield(k{i},'Yest');
end
...
k{1}.tolX = 1e-16; k{1}.tolFun = 1e-16; % Optimization options
