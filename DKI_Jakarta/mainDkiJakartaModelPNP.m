%% Model PositifNonpositif (PNP) DKI Jakarta per Kebijakan:
...
% Compartments: 
% S: Susceptible
% NI: Nonpositive Isolate (Nonpositif Isolasi)
% NR: Nonpositive Recovered (Nonpositif Selesai Isolasi)
% P: Positive Isolate (Positif Isolasi)
% R: Recovered (Sembuh)
% ND: Nonpositive Dead (Nonpositif Meninggal)
% D: Dead (Positif Meninggal)

%By: Bagaskara P.P., Last Modified: 2020-09-07
...
% 1. Tanpa Kebijakan (1 Maret - 18 Maret)
% 2. Larangan Kerumunan Maklumat POLRI No Mak/III/2020 (19 Maret - 9 April)
% 3. PSBB Permenkesri No. 9 Tahun 2020 (10 April - 4 Juni)
% 4. PSBB Masa Transisi (5 Juni - ...)
...
k{1}.startDate = '2020-03-01'; k{1}.endDate = '2020-03-18'; k{1}.numDays = 1;
k{2}.name = 'Larangan Kerumunan Maklumat POLRI'; 
k{2}.startDate = '2020-03-19'; k{2}.endDate = '2020-04-09'; k{2}.numDays = 1;
k{3}.name = 'PSBB Permenkesri No. 9 Tahun 2020'; 
k{3}.startDate = '2020-04-10'; k{3}.endDate = '2020-06-04'; k{3}.numDays = 1;
k{4}.name = 'PSBB Masa Transisi Nonpositif Tanpa Meninggal'; 
k{4}.startDate = '2020-06-05'; k{4}.endDate = '2020-06-18'; k{4}.numDays = 1;
k{5}.name = 'PSBB Masa Transisi Istilah Lama'; 
k{5}.startDate = '2020-06-19'; k{5}.endDate = '2020-07-16'; k{5}.numDays = 1;
k{6}.name = 'PSBB Masa Transisi Istilah Baru'; 
k{6}.startDate = '2020-07-17'; k{6}.endDate = '2020-08-27'; k{6}.numDays = 14;
k{7}.name = 'Long Weekend 28 Agustus 2020'; 
k{7}.startDate = '2020-08-28';
rfi = numel(k); % real fitting index: indeks kebijakan terakhir yang merupakan data fitting nyata

lockdown.index = rfi+1;
lockdown.startDate = '2020-09-05'; % pertimbangan kapasitas RS
% lockdown.startDate = '2020-10-31'; %25%
% lockdown.startDate = '2020-11-14';  %50%
% lockdown.startDate = '2020-11-24'; %75%
...
if(softwareName == 'matlab')
    lama = getDataIstilahLamaModelPNP('COVID19_DkiJakarta_IstilahLama.xls'); %[EDITABLE Matlab]
    baru = getDataIstilahBaruModelPNP('COVID19_DkiJakarta_IstilahBaru.xls'); %[EDITABLE Matlab]
else
    lama = getDataIstilahLamaModelPNP('COVID19_DkiJakarta_IstilahLama.csv'); %[EDITABLE Octave]
    baru = getDataIstilahBaruModelPNP('COVID19_DkiJakarta_IstilahBaru.csv'); %[EDITABLE Octave]
end
...
stateLineProp(2,:) = {yellowDef,'-',1.5};
stateLineProp(3,:) = {purpleDef,'-',1.5};
stateLineProp(4,:) = {'m','-',1.5};
stateLineProp(5,:) = {greenDef,'-',1.5};
stateLineProp(6,:) = {brownDef,'-',1.5};
...
for i=1:numel(k)
    ...
    if(i==lockdown.index || i==lockdown.index+1)
        ...
        k{i}.stateLineProp(2,:) = {yellowDef,'--',1.5};
        k{i}.stateLineProp(3,:) = {purpleDef,'--',1.5};
        k{i}.stateLineProp(4,:) = {'m','--',1.5};
        k{i}.stateLineProp(5,:) = {greenDef,'--',1.5};
        k{i}.stateLineProp(6,:) = {brownDef,'--',1.5};
        k{i}.stateLineProp(7,:) = {'r','--',1.5};
    end
end
...
k{6}.vlColor = orangeDef;
k{7}.vlColor = 'k';
k{8}.vlColor = brownDef;
k{9}.vlColor = blueDef;
...
cursorIndexMax{1}.stateName = 'P'; % nama state yang akan diberi cursor
cursorIndexMax{2}.kebijakan = lockdown.index+1; % indeks kebijakan pasca-lockdown yang akan diberi cursor
cursorIndexMax{2}.stateName = 'P'; % nama state yang akan diberi cursor
...
% custom states
customStates = {'P','R','D'}; % [EDITABLE] Ubah nama state sesuai yang ingin di-plot. Pisahkan dengan koma ','.
