%% SIRQ DKI Jakarta per Kebijakan with Semi Lockdown:
...
%By: Bagaskara P.P., Last Modified: 2020-11-22
...
rfldi = 2; % real fitting lockdown index:  indeks kebijakan terakhir yang merupakan data fitting kebijakan PSBB total nyata

% [EDITABLE] Comment lockdown & postlockdown if you do not want to simulate lockdown
lockdown.index = rfi+1;
lockdown.startDate = '2020-12-18'; %25%
% lockdown.startDate = '2021-02-11'; %50%
% lockdown.startDate = '2021-03-25'; %75%
lockdown.numDays = 14; % durasi simulasi lockdown total
postlockdown.numDays = 730; %365; % durasi simulasi pasca lockdown
if(exist('lockdown'))
    k{lockdown.index}.name = 'PSBB Total Simulation';
...
if(exist('postlockdown'))
    k{lockdown.index+1}.name = 'PSBB Transisi Simulation';
...
if(exist('lockdown'))
    k{rfldi-1}.numDays = datenum(k{lockdown.index}.timeFit{1}-k{rfldi-1}.timeFit{end})+lockdown.numDays+postlockdown.numDays; 
...
if(exist('lockdown'))
   keteranganSimulasi = [keteranganSimulasi ...
                        'SemiLockdown' lockdown.startDate 'Duration' num2str(lockdown.numDays)];
end
...
if(exist('lockdown'))
    ...
    if(datenum(k{lockdown.index}.startDate) > datenum(k{rfldi-1}.timeFit{end})) % jika tanggal lockdown melebihi tanggal akhir fitting
    % nilai awal kebijakan lockdown = nilai dari data simulasi kebijakan sebelumnya pada tanggal tsb.
        for i=1:numel(model.allStateName)
            k{lockdown.index}.y0(i) = ...
                k{rfldi-1}.Yest((findIndexFromCell(k{rfldi-1}.timeSim,datetime(k{lockdown.index}.startDate))), i);
        end
    ...
    k{lockdown.index}.paramEst = k{rfldi}.paramEst; % set parameter sama seperti kebijakan sebelumnya,
%     k{lockdown.index}.paramEst(find(strcmp(model.paramName, 'beta'))) = 0; % namun nilai beta di-set nol
    ...
    k{lockdown.index+1}.paramEst = k{rfldi-1}.paramEst; % set parameter sama seperti kebijakan sebelum lockdown,
    k{lockdown.index+1} = simulateModel(k{lockdown.index+1},k{lockdown.index+1}.paramEst,model);
end

% Remove PSBB Total fitting data, replace with simulated PSBB Total 
sizeBeforeReplaced = numel(k);
krfiNew = k(1:rfldi-1); kldNew = k(lockdown.index:sizeBeforeReplaced);
k = [krfiNew kldNew];
rfi = rfldi-1; lockdown.index = lockdown.index-1;
